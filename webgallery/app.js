...
function setCookie(res, userId) {
	res.setHeader('Set-Cookie', cookie.serialize('username', userId, {
		path: '/',
		maxAge: 60 * 60 * 24 * 7
	}));
}
...
// Get profile image
app.get('/api/images/:imageId/image', isAuthenticated, function (req, res, next) {
	images.findOne({
...
app.get('/api/images', isAuthenticated, function (req, res) {
	...
	images.count({ author: req.query.username }, (err, count) => {
		images.find({ author: req.query.username })
			.sort({createdAt: -1})
			.skip(page)
			.limit(1)
			.exec((err, image) => {
...
app.patch('/api/comments/:id/', isAuthenticated, function (req, res, next) {
	...
	switch (req.body.action) {
		case 'upvote':
			comments.update({ _id: id, author: req.session.username }, { $inc: { upvote: 1 }}, handle);
			break;
		case 'downvote':
			comments.update({ _id: id, author: req.session.username  }, { $inc: { downvote: 1} }, handle);
			break;
		default:
...
app.delete('/api/comments/:id/', isAuthenticated, function (req, res, next) {
	...
	let handle = function(err, comment) {
		comments.count({ imageId: comment.imageId, author: req.session.username }, (err, count) => {
			if (err) return res.status(500).send(err);
			res.json({
				msg: 'Deleted comment',
				remaining: count - 1,
		...
		if (!comment) {
			...
			images.findOne({ _id: comment.imageId }, (_, image) => {
				if (!image) {
					...
					if (image.author == req.session.username || req.session.username == comment.author) {
...
app.delete('/api/images/:id', isAuthenticated, function (req, res, next) {
	...
	images.findOne({ _id: id }, (_, image) => {
		if (!image) {
			...
			images.remove({ _id: id, author: req.session.username }, {}, handle(_, image));
		}
	});
});
...
app.post('/signup/', function (req, res, next) {
  ...
  users.findOne({ _id: username }, (err, user) => {
    ...
    hashPassword(password, (_err, hash) => {
      users.update({ _id: username }, { _id: username, password: hash }, { upsert: true }, (err) => {
      if (err) return res.status(500).end(err);
